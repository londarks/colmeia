...
export function WorkspacePanel({
  ...
  const [menu, setMenu] = useState<
    { x: number; y: number; workspace?: string; folder?: string } | null
  >(null);
    ...
    return (
      <div key={ws.id} className="ws-row-wrap">
        <div
          ...
          draggable={!isEditing}
          onDragStart={(e) => {
            // Q11: arrastar item fora da seleção reseta a seleção para só ele.
            if (!selWs.has(ws.id)) {
          ...
          onClick={(e) => {
            ...
            clearSelection(); // Q7: clique simples colapsa a seleção
    ...
    <div className="ws-tree">
      ...
      {menu && (
        ...
        >
          ...
          {selectionCount >= 2 ? (
            // Q10: seleção múltipla → ação em lote.
            <button
